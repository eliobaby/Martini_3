def get_m3_dict():
    # 1) Start by listing all (key, value) pairs in one big list.
    #    Even if a key repeats in this list, we’ll handle collisions when inserting into the dict.
    entries = [
        # Section 1: Benzene nodes
        ("TN6a", [1, 0, "CN", 2, ""]),
        ("TC5",  [1, 0, "CC", 2, ""]),
        ("TN3a",  [1, 0, "NN", 2, ""]),

        # Section 2: Benzene border nodes
        ("SN2",   [2, 0, "CC(O)",   2, ""]),
        ("SN4d",  [2, 0, "CC(N)",   2, ""]),
        ("SC6",   [2, 0, "CC(S)",   2, ""]),
        ("SX3",   [2, 0, "CC(Cl)",  2, ""]),
        ("SX3e",  [2, 0, "CC(F)",   2, ""]),
        ("X1",    [2, 0, "CC(I)",   2, ""]),
        ("SX1",   [2, 0, "CC(Br)",  2, ""]),
        ("SC4",   [2, 0, "CC(C)",   2, ""]),
        ("SP1a",  [2, 0, "CC(=O)",  2, ""]),
        ("TN2",   [2, 0, "C(O)",    2, ""]),
        ("TX4",   [2, 0, "C(Cl)",   2, ""]),
        ("TN6d",  [2, 0, "C(N)",    2, ""]),
        ("TC6",   [2, 0, "C(S)",    2, ""]),
        ("TX1",   [2, 0, "C(I)",    2, ""]),
        ("TX1",   [2, 0, "C(Br)",   2, ""]),
        ("TC6",   [2, 0, "C(C)",    2, ""]),
        ("TN2a",  [2, 0, "C(=O)",   2, ""]), #correct
        ("TX4e",  [2, 0, "C(F)",    2, ""]),
        ("SN1a",  [2, 0, "C(OC)",   2, ""]),

        # Section 3: Non‑benzene 6‑ring nodes
        ("SN4a", [3, 0, "COC", 2, "SC3"]),
        ("SN3a", [3, 0, "COC", 2, "SN3a"]),
        ("SC3",  [3, 0, "CCC", 2, ""]),

        # Section 4: Non‑benzene 6‑ring border nodes
        ("SN5a", [4, 0, "OCO", 2, ""]),
        ("SN1",  [4, 0, "NCN", 2, ""]),
        ("SX4e", [4, 0, "FCF", 2, ""]),
        ("SN3a", [4, 0, "COC", 2, ""]),

        # Section 5: 5‑ring nodes
        ("TN6d", [5, 0, "NH",  2, ""]),
        ("SN5a", [5, 0, "OCO", 2, ""]),
        ("SC6",  [5, 0, "CSC", 2, ""]),
        ("TP6a", [5, 0, "OC",  2, ""]),
        ("TC3",  [5, 0, "CC",  2, ""]),
        ("TN3a", [5, 0, "O",   2, ""]),
        ("TN2",  [5, 0, "S",   2, ""]), #immovable
        ("TC6",  [5, 0, "SS",  2, ""]),

        # Section 6: 5‑ring border nodes
        ("SN6", [6, 0, "CC(O)", 2, ""]),
        ("SC3", [6, 0, "CC(C)", 2, ""]),
        ("SP6", [6, 0, "CC(N)", 2, ""]),
        ("TN3", [6, 0, "N(C)",  2, ""]),

        # Section 7: Non‑ring nodes (polar/apolar fragments)
        ("X1",   [7, 2, "C(Br)(Br)(Br)(Br)", 0, "."]),
        ("X2",   [7, 2, "C(Cl)(Cl)(Cl)(Cl)", 0, "."]),
        ("X4",   [7, 2, "C(F)(F)(F)(F)"    , 0, "."]),
        ("C1",   [7, 2, "C(C)(C)(C)(C)"    , 0, ""]),
        ("X3",   [7, 2, "C(C)(C)(C)(Cl)"   , 0, ""]),
        ("X2",   [7, 2, "C(C)(C)(C)(Br)"   , 0, ""]),
        ("X1",   [7, 2, "C(C)(C)(C)(I)"    , 0, ""]),
        ("P3",   [7, 1, "C(O)(=O)(CC)",   0, ""]),
        ("P3",   [7, 1, "C(O)(=O)(C=C)",  0, ""]),
        ("SN5",  [7, 1, "C(O)(=O)(C)",    0, ""]),
        ("SP2",  [7, 1, "C(O)(=O)(N)",    0, ""]),
        ("N2",   [7, 1, "C(O)(C)(C)",     0, ""]),
        ("N2",   [7, 1, "C(O)(C)(CC)",    0, ""]),
        ("N2",   [7, 1, "C(C)(C)(CO)",    0, ""]),
        ("SP4",  [7, 1, "C(O)(O)(C)",     0, ""]),
        ("P4",   [7, 1, "C(O)(OC)(C)",    0, ""]),
        ("SN5",  [7, 1, "C(N)(=O)(C)",    0, ""]),
        ("P4",   [7, 1, "C(NC)(=O)(C)",   0, ""]), #immovable
        ("P6",   [7, 1, "C(NC)(=O)(O)",   0, ""]),
        ("P4a",  [7, 1, "N(C=O)(C)(C)",   0, ""]),
        ("P4",   [7, 1, "S(=O)(C)(C)",    0, ""]), #immovable
        ("P6",   [7, 1, "S(=O)(=O)(O)",   0, ""]),
        ("SN5a", [7, 1, "C(=O)(C)(C)",    0, ""]),
        ("N5a",  [7, 1, "C(=O)(C)(CC)",   0, ""]),
        ("SC3",  [7, 1, "C(C)(C)(C)",     0, ""]),
        ("C2",   [7, 1, "C(CC)(C)(C)",    0, ""]),
        ("SC4",  [7, 1, "C(=C)(C)(C)",    0, ""]),
        ("C4",   [7, 1, "C(=CC)(C)(C)",   0, ""]),
        ("C4",   [7, 1, "C(=C)(CC)(C)",   0, ""]),
        ("C4",   [7, 1, "C(=C)(C=C)(C)",  0, ""]),
        ("SN2a", [7, 1, "N(C)(C)(C)",     0, ""]),
        ("N3a",  [7, 1, "N(C)(C)(CC)",    0, ""]),
        ("N5a",  [7, 1, "C(C)(=O)(OC)",    0, ""]),
        ("N5a",  [7, 1, "C(C)(=O)(CO)",    0, ""]),
        ("N1a",  [7, 1, "C(C)(C)(=NN)",    0, ""]),
        ("SN2",  [7, 1, "C(C)(C)(=N)",     0, ""]),
        ("SN2ah",[7, 1, "C(C)(=O)(Cl)",    0, ""]),
        ("SX4e", [7, 1, "C(F)(F)(F)",      1, ""]),
        ("X2",   [7, 1, "C(Cl)(Cl)(Cl)",   0, ""]),
        ("SX3h", [7, 1, "C(C)(Cl)(Cl)",    0, ""]),
        ("SX4e", [7, 1, "C(C)(C)(F)",      0, ""]),
        ("X1",   [7, 1, "C(C)(C)(I)",      0, ""]),
        ("SX1",  [7, 1, "C(C)(C)(Br)",     0, ""]),
        ("SC6",  [7, 1, "C(C)(C)(S)",      0, ""]),
        ("SX3",  [7, 1, "C(C)(C)(Cl)",     0, ""]),
        ("SN2r", [7, 0, "COC",          0, ""]),
        ("N5a",  [7, 0, "O=CCO",        0, ""]),
        ("TP1d", [7, 0, "CO",           1, "SX4e"]),
        ("P4",   [7, 0, "OCCO",         0, ""]), #immovable
        ("P1",   [7, 0, "O=CNC",        0, ""]), #correct
        ("SP6",  [7, 1, "CS=O",         0, ""]),
        ("TN4a", [7, 0, "CO",           1, "TC5"]), #correct
        ("SN2a", [7, 0, "N#CC",         1, ""]),
        ("N5a",  [7, 0, "CCC=O",        1, "C1"]),
        ("N2",   [7, 0, "CCCO",         1, ""]),
        ("N5a",  [7, 0, "COC=O",        1, ""]),
        ("SN2",  [7, 0, "CC=O",         1, ""]),
        ("SN2",  [7, 0, "CCO",          1, ""]), #immovable
        ("SP3a", [7, 0, "OC=O",         1, ""]), #immovable or SP4a
        ("SP3a", [7, 0, "NC=O",         1, ""]), #immovable or SP4a
        ("TN4a", [7, 0, "N#C",          1, ""]),
        ("TN2",  [7, 0, "C=O",          1, ""]), #immovable
        ("SN3a", [7, 0, "ON=O",         1, ""]),
        ("N1a",  [7, 0, "CC=NN",        0, ""]),
        ("N1a",  [7, 0, "CC=NC",        0, ""]),
        ("SN1a", [7, 0, "C=NN",         0, ""]),
        ("SN2",  [7, 0, "CC=N",         0, ""]),
        ("TN2",  [7, 0, "C=N",          0, ""]),
        ("SN2",  [7, 0, "C=C=N",        0, ""]),
        ("TP1",  [7, 0, "CN",           0, ""]),
        ("SN5",  [7, 0, "CCN",          0, ""]),
        ("SP1",  [7, 0, "C=CN",         0, ""]),
        ("N5",   [7, 0, "CNCC",         0, ""]), #or N4
        ("P3",   [7, 0, "CCCN",         0, ""]), 
        ("SN2ah",[7, 0, "O=CCl",        0, ""]),
        ("SN2ah",[7, 0, "ClC=O",        0, ""]),
        ("SN5",  [7, 0, "CNC",          1, ""]),
        ("TC5",  [7, 0, "C=C",          1, ""]),
        ("TC3",  [7, 0, "CC",           1, ""]),
        ("C6r",  [7, 0, "CC#CC",        0, ""]),
        ("C6r",  [7, 0, "C#CCC",        0, ""]),
        ("C6r",  [7, 0, "C#CC=C",       0, ""]),
        ("C6r",  [7, 0, "C#CC#C",       0, ""]),
        ("SC6r",[7, 0, "C#CC",         0, ""]),
        ("TC6r",[7, 0, "C#C",          0, ""]),
        ("SC6", [7, 0, "CSC",          0, ""]),
        ("C4",  [7, 0, "CC=CC",        0, ""]),
        ("C6",  [7, 0, "C=CCC",        0, ""]),
        ("SC4", [7, 0, "C=CC",         0, ""]),
        ("SC4", [7, 0, "C=C=C",        0, ""]),
        ("C3",  [7, 0, "CCCC",         0, ""]),
        ("X2",  [7, 0, "ClCCCl",       0, ""]),
        ("SX3h",[7, 0, "ClCCl",        0, ""]),
        ("TX3h",[7, 0, "CCl",          0, ""]),
        ("TX3h",[7, 0, "ClC",          0, ""]),
        ("SC3", [7, 0, "CCC",          1, ""]),
        ("SX4e",[7, 0, "FCF",          1, ""]),
        ("SC6", [7, 0, "CCS",          2, ""]),
        ("SX3", [7, 0, "CCCl",         2, ""]),
        ("SX3", [7, 0, "ClCC",         2, ""]),
        ("SX4e",[7, 0, "CCF",          2, ""]),
        ("X1",  [7, 0, "CCI",          2, ""]),
        ("SX1", [7, 0, "CCBr",         2, ""]),
        ("SX1", [7, 0, "BrCC",         2, ""]),
        ("TC6", [7, 0, "CS",           2, ""]),
        ("TX1", [7, 0, "CI",           2, ""]),
        ("TX1", [7, 0, "CBr",          2, ""]),
        ("TX1", [7, 0, "BrC",          2, ""]),
        ("X1",  [7, 0, "BrCCBr",       0, ""]),
        ("TX3e",[7, 0, "CF",           2, ""]), #correct -0.117

        # Section 8: node that is in 2 ben?ene rings
        ("TC5e", [8, 2, "CC", 4, ""]),
        ("TC5",  [8, 1, "CC", 4, ""]),

        # Section 9: 3/4/7/8‑rings section
        ("SC3",  [9, 0, "CCC", 2, ""]),
        ("SN4a", [9, 0, "COC", 2, ""]),

        # Section 10: Singular atom section
        ("TC6",  [10, 0, "S", 0, ""]),

        # Section 11: Structures that are forced
        ("P3?",   [11, 2, "S(C)(=O)(=O)(C)"  , 0, ""]),
        ("C2?",   [11, 2, "Ge(C)(C)(C)(C)"   , 0, ""]),
        ("C2?",   [11, 2, "Pb(C)(C)(C)(C)"   , 0, ""]),
        ("C2?",   [11, 2, "Si(C)(C)(C)(C)"   , 0, ""]),
        ("C2?",   [11, 2, "Sn(C)(C)(C)(C)"   , 0, ""]),
        ("SC2?",  [11, 1, "Si(C)(C)(C)",    0, ""]),
        ("P2?",   [11, 1, "C(O)(=O)(CO)",   0, ""]),
        ("SP3?",  [11, 1, "S(=O)(=O)(C)",   0, ""]),
        ("P2?",   [11, 1, "C(O)(=O)(CN)",   0, ""]),
        ("SX4e?", [11, 1, "C(Cl)(F)(F)",    1, ""]),
        ("X4e?",  [11, 1, "C(OC)(F)(F)",    1, ""]),
        ("X4e?",  [11, 1, "C(CO)(F)(F)",    1, ""]),
        ("SX4e?", [11, 1, "C(O)(F)(F)",     1, ""]),
        ("X4e?",  [11, 1, "C(=C)(F)(F)",    1, ""]),
        ("X4e?",  [11, 1, "C(=CF)(F)(F)",   1, ""]),
        ("X4e?",  [11, 1, "C(Cl)(CF)(F)",   1, ""]),
        ("X4e?",  [11, 1, "C(=O)(CF)(N)",   1, ""]),
        ("X2?",   [11, 1, "C(F)(Cl)(Cl)",   0, ""]),
        ("X2?",   [11, 1, "C(CCl)(Cl)(Cl)", 0, ""]),
        ("X2?",   [11, 1, "C(=CCl)(Cl)(Cl)",0, ""]),
        ("SX2?",  [11, 1, "C(C)(Cl)(Cl)",   0, ""]),
        ("SX2?",  [11, 1, "C(C)(Cl)(CCl)",  0, ""]),
        ("SX2?",  [11, 1, "C(=C)(Cl)(Cl)",  0, ""]),
        ("X1?",   [11, 1, "C(Br)(Cl)(Br)",  0, ""]),
        ("X1?",   [11, 1, "C(Br)(Br)(Br)",  0, ""]),
        ("X1?",   [11, 1, "C(=CBr)(Br)(Br)",0, ""]),
        ("X2?",   [11, 1, "C(Cl)(Cl)(Br)",  0, ""]),
        ("SP2?",  [11, 1, "C(C)(=O)(=O)",   0, ""]),
        ("N5a?",  [11, 1, "C(C)(C=O)(=C)",  0, ""]),
        ("N5?",   [11, 1, "C(=C)(C)(CO)",   0, ""]),
        ("P4?",   [11, 1, "C(O)(C)(CO)",    0, ""]),
        ("SN4?",  [11, 1, "N(O)(C)(C)",     0, ""]),
        ("SN4?",  [11, 1, "N(N=O)(C)(C)",   0, ""]),
        ("SP4?",  [11, 1, "C(O)(=O)(=O)",   0, ""]),
        ("P5?",   [11, 1, "C(CN)(=O)(N)",   0, ""]),
        ("P6?",   [11, 1, "C(OC)(=O)(N)",   0, ""]),
        ("P5?",   [11, 1, "C(NC)(=O)(N)",   0, ""]),
        ("SP5?",  [11, 1, "C(N)(=O)(N)",    0, ""]),
        ("P6?",   [11, 1, "C(NO)(=O)(N)",   0, ""]),
        ("P5?",   [11, 1, "C(NO)(=O)(C)",   0, ""]),
        ("P5?",   [11, 1, "C(NN)(=O)(C)",   0, ""]),
        ("SP5?",  [11, 1, "C(N)(=S)(N)",    0, ""]),
        ("P6?",   [11, 1, "C(N)(=O)(NN)",   0, ""]),
        ("SP5?",  [11, 1, "C(N)(=N)(N)",    0, ""]),
        ("X3?",   [11, 1, "C(C)(C)(CCl)",   0, ""]),
        ("X3?",   [11, 1, "C(O)(C)(CCl)",   0, ""]),
        ("SX3?",  [11, 1, "C(C)(=C)(Cl)",   0, ""]),
        ("X3?",   [11, 1, "C(C)(CC)(Cl)",   0, ""]),
        ("X2?",   [11, 1, "C(C)(C)(CBr)",   0, ""]),
        ("SC3?",  [11, 1, "Pb(C)(C)(C)",    0, ""]),
        ("SN4a?", [11, 0, "NCO",           1, ""]),
        ("TP4?",  [11, 0, "OO",            0, ""]),
        ("TN3a?", [11, 0, "NO",            0, ""]),
        ("TN3a?", [11, 0, "N=O",           0, ""]),
        ("TN1a?", [11, 0, "NN",            0, ""]),
        ("TC5?", [11, 0, "N=N",           0, ""]),
        ("SN1a?", [11, 0, "CNN",           0, ""]),
        ("SN2?",  [11, 0, "NC=S",          0, ""]),
        ("SN2?",  [11, 0, "N=C=S",         0, ""]),
        ("SN2?",  [11, 0, "NC=N",          0, ""]),
        ("SP2?",  [11, 0, "O=C=O",         0, ""]),
        ("SP1?",  [11, 0, "C=C=O",         0, ""]),
        ("SP6?",  [11, 0, "O=S=O",         0, ""]),
        ("TC6?",  [11, 0, "C=S",           0, ""]),
        ("SC6?",  [11, 0, "CS=O",          0, ""]),
        ("SC6?",  [11, 0, "S=C=S",         0, ""]),
        ("SN2?",  [11, 0, "S=PC",          0, ""]),
        ("SP4?",  [11, 0, "OCO",           0, ""]),
        ("SN2a?", [11, 0, "C=CO",          0, ""]),
        ("SX2?",  [11, 0, "C=CBr",         2, ""]),
        ("SX2?",  [11, 0, "BrCBr",         0, ""]),
        ("SX1?",  [11, 0, "ICI",           0, ""]),
        ("SX3h?", [11, 0, "ClCBr",         0, ""]),
        ("SX3h?", [11, 0, "BrCCl",         0, ""]),
        ("SX3h?", [11, 0, "ClCF",          0, ""]),
        ("SX3h?", [11, 0, "ClHg",          0, ""]),
        ("SX3h?", [11, 0, "HgCl",          0, ""]),
        ("SX3h?", [11, 0, "ClN",           0, ""]),
        ("SX3h?", [11, 0, "NCl",           0, ""]),
        ("SX3h?", [11, 0, "FCCl",          0, ""]),
        ("SX3?",  [11, 0, "C=CCl",         0, ""]),
        ("SX3?",  [11, 0, "ClC=C",         0, ""]),
        ("SX3?",  [11, 0, "ClCO",          0, ""]),
        ("SC3?",  [11, 0, "CPbC",          0, ""]),
        ("SC3?",  [11, 0, "CSiC",          0, ""]),

        # Section 12: Structure that are formed from a merge
        ("N5",  [12, 0, "OCCO", 0, ""]),
        ("N5a", [12, 0, "OCOC", 0, ""]),
    ]

    # 2) Create the final dictionary, inserting each entry in order.
    #    If a key already exists, append “+” until it’s unique.
    martini_dict = {}
    for key, val in entries:
        new_key = key
        while new_key in martini_dict:
            new_key = new_key + "+"
        # Use a copy of val (so mutating one list doesn’t affect others, if ever)
        martini_dict[new_key] = list(val)

    return martini_dict
